    ...
    ? trips
        .map(
          (t) => `<tr>
            ...
            <td data-column="name"><a class="row-link" href="/trips/${t.id}">${escapeHtml(t.name)}</a><span class="cell-sub">${formatDate(t.start_date)} &ndash; ${formatDate(t.end_date)}${t.description ? ` &middot; ${escapeHtml(t.description)}` : ''}</span></td>
